...
<head>
    <meta charset="UTF-8">
    <title>${i18n.get('address.editTitle')}</title>
    <style>
...
<%
    ...
    int userId = address != null ? address.getUserId() : 0;
%>

<h1>${i18n.get('address.editTitle')}</h1>
<nav>
    <%@ include file="/WEB-INF/includes/language-switcher.jsp" %>
    <a href="${pageContext.request.contextPath}/admin/addresses?userId=<%= userId %>">← ${i18n.get('address.backToList')}</a>
</nav>
<br>
...
<form method="post" action="${pageContext.request.contextPath}/admin/addresses">
    <input type="hidden" name="action" value="edit">
    <input type="hidden" name="id" value="<%= address != null ? address.getId() : "" %>">
    ...
    <div class="form-group">
        ...
        <select id="provinceId" name="provinceId" required>
            ...
            %>
            <option value="<%= p.getId() %>" <%= selected ? "selected" : "" %>><%= p.getLocalizedName(lang) %></option>
            <% } } %>
        </select>
    </div>
    ...
    <div class="actions">
        ...
        <a href="${pageContext.request.contextPath}/admin/addresses?userId=<%= userId %>" class="btn btn-cancel">${i18n.get('action.cancel')}</a>
    </div>
</form>
</body>
</html>
